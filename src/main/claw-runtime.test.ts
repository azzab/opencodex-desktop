...
import { createClawRuntime, registerFeishuReadReceiptNoop } from './claw-runtime'
...
describe('ClawRuntime', () => {
  it('registers a no-op Feishu read receipt handler when the dispatcher exists', () => {
    const register = vi.fn()
    registerFeishuReadReceiptNoop({
      dispatcher: { register }
    })
    ...
    expect(() => {
      const handler = register.mock.calls[0]?.[0]?.['im.message.message_read_v1']
      handler?.({ event: 'read' })
    }).not.toThrow()
  })
