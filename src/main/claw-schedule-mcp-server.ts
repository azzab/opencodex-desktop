...
export async function runClawScheduleMcpServerFromArgv(argv: string[]): Promise<boolean> {
  ...
  const server = new McpServer(
    { name: 'opencodex-desktop-schedule', version: '0.1.0' },
  ...
  const registerListTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. List scheduled tasks managed by the currently running OpenCodex Desktop app.'
        : 'List scheduled tasks managed by the currently running OpenCodex Desktop app.'
    }, async () => {
      try {
  ...
  const registerCreateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Create a scheduled task in OpenCodex Desktop. Supports one-time (`at`), daily, or interval schedules.'
        : 'Create a scheduled task in OpenCodex Desktop. Supports one-time (`at`), daily, or interval schedules.',
      inputSchema: {
  ...
  const registerUpdateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Update an existing OpenCodex Desktop scheduled task.'
        : 'Update an existing OpenCodex Desktop scheduled task.',
      inputSchema: {
  ...
  const registerDeleteTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Delete a scheduled task from OpenCodex Desktop.'
        : 'Delete a scheduled task from OpenCodex Desktop.',
      inputSchema: {
        task_id: z.string().min(1).describe('Task id returned by gui_schedule_list or gui_schedule_create')
      }
    }, async ({ task_id }) => {
