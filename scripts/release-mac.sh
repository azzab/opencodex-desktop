...
#   OPENCODEX_DESKTOP_RUNTIME_CACHE=0    disable bundled runtime cache
#
...
build_mac_arch() {
  ...
  OPENCODEX_DESKTOP_DIST_DIR="${output_dir}" \
...
copy_mac_arch_artifacts() {
  ...
  shopt -s nullglob
  files=("${output_dir}"/OpenCodex-Desktop-*-mac-"${arch}".*)
  shopt -u nullglob
...
collect "macOS arm64 dmg" "dist/OpenCodex-Desktop-*-mac-arm64.dmg"
collect "macOS x64 dmg" "dist/OpenCodex-Desktop-*-mac-x64.dmg"
collect "macOS arm64 zip" "dist/OpenCodex-Desktop-*-mac-arm64.zip"
collect "macOS x64 zip" "dist/OpenCodex-Desktop-*-mac-x64.zip"
collect_optional "macOS blockmap" "dist/OpenCodex-Desktop-*-mac-*.zip.blockmap"
...
```sh
xattr -cr "OpenCodex Desktop.app"
# or
...
cyan "  https://github.com/azzab/opencodex-desktop/releases/tag/${TAG_NAME}"
