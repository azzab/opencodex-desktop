...
const version = (
  process.env.OPENCODEX_DESKTOP_APP_VERSION || process.env.DEEPSEEK_GUI_APP_VERSION || pkg.version || ''
).trim()
if (!version) {
...
const distDir = resolve(process.env.OPENCODEX_DESKTOP_DIST_DIR || process.env.DEEPSEEK_GUI_DIST_DIR || join(root, 'dist'))
const appOutDir = join(distDir, arch === 'arm64' ? 'mac-arm64' : 'mac')
const appName = 'OpenCodex Desktop.app'
const appPath = join(appOutDir, appName)
const zipPath = join(distDir, `OpenCodex-Desktop-${version}-mac-${arch}.zip`)
