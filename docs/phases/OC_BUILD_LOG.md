...
| M3 | IDE Experience Parity | `oc-m3-ide` | dsv4-pro max | 🟡 | $0.6353 total: initial $0.5290 / in=362,197 out=335,026 cacheRead=22,068,224 cache 98.4%; lint remediation $0.1063 / in=186,291 out=19,174 cacheRead=2,370,688 cache 92.7% | Initial gate failed at root `npm run lint` on `clients/vscode/webview/sidebar.js` missing an `acquireVsCodeApi` global declaration. Lint remediation then passed the full independent gate (root typecheck/lint/tests 154 files/1317 tests, Kun typecheck/tests 58 files passed / 1 skipped and 658 tests passed / 4 skipped, build, `git diff --check`, VS Code typecheck/tests/build/package), but source review found fake Antigravity workaround settings documented in `PUBLISHING.md` without manifest/code support. Second remediation ordered before merge. | — | Evidence paths included: `docs/reference-evidence/ide/`; Antigravity hang root-cause required |
