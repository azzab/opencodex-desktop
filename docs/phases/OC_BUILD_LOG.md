...
| 9 | H9 CLI Binary & IDE Extension | `oc-h9-clients` | dsv4-pro high | 🟡 | initial READY rejected $0.4439 / in 426,260 out 188,447 cacheRead 26,083,712 cache 98.4%; remediation1 rejected $0.4357 / in 538,083 out 121,456 cacheRead 26,484,736 cache 98.0%; remediation2 ordered | Initial H9 full gate failed independently at root `npm test` (8 Electron import suites failed); remediation1 full gate green but stop-gate review rejected: `opencodex serve` only prints a hint, CLI smoke accepts missing real marker/approval as pass, no real VS Code/manual parity proof | — | Remediation2 same id / same dirty tree; H9 must prove real `opencodex serve` + real streamed turn + real approval allow/deny or explicitly block on an operator-only credential/runtime prerequisite |
