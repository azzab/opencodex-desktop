...
| M3 | IDE Experience Parity | `oc-m3-ide` | dsv4-pro max | 🟡 | $0.5290 / in=362,197 out=335,026 cacheRead=22,068,224 cache 98.4% | Worker reported READY, but orchestrator gate failed at root `npm run lint`: `clients/vscode/webview/sidebar.js` referenced the VS Code webview global `acquireVsCodeApi` without a lint-safe declaration. Dependency install was required in the worktree before the gate could run. Remediation ordered before merge. | — | Evidence paths included: `docs/reference-evidence/ide/`; Antigravity hang root-cause required |
