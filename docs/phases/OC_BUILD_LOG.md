...
| 7 | H7 Hooks Execution & Trust | `oc-h7-hooks` | dsv4-pro max | 🟡 | initial READY rejected: $0.5901 / in 490,731 out 217,334 cacheRead 51,750,016 cache 99.1%; remediation1 rejected: $0.6608 / in 606,142 out 194,460 cacheRead 62,872,832 cache 99.0% | Full command gate green in remediation1, but orchestrator stop-gate review rejected before merge because persisted trust settings do not reach the managed Kun runtime | — | Hook execution defaults disabled in `createKunServeRuntime` unless `options.hookSettings` is passed; no app/runtime bridge passes `agents.kun.hooks`; remediation2 required |
