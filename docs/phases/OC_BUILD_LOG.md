...
| 9 | H9 CLI Binary & IDE Extension | `oc-h9-clients` | dsv4-pro high | ❌ | initial READY rejected $0.4439 / in 426,260 out 188,447 cacheRead 26,083,712 cache 98.4%; remediation1 rejected $0.4357 / in 538,083 out 121,456 cacheRead 26,484,736 cache 98.0%; remediation2 blocked $0.4203 / in 453,945 out 139,471 cacheRead 28,002,176 cache 98.4% | Remediation2 full gate green, but independent CLI smoke failed at the required real streamed-turn stop gate: provider/model auth returned 401, so no `CLI_SMOKE_OK` marker turn completed. `opencodex serve` and insecure-only approval fixture source reviewed. | — | Operator/model credential prerequisite required before H9 can merge; worktree retained at `../ocx-h9` |
| 10 | H10 SSH Remote Runner | `oc-h10-ssh` | dsv4-pro max | 🟡 | initial READY rejected $0.7052 / in 657,664 out 237,472 cacheRead 58,612,864 cache 98.9%; remediation1 rejected $0.8065 / in 1,051,583 out 172,136 cacheRead 54,973,184 cache 98.1%; remediation2 ordered | Initial H10 full gate green in worktree (root 1029/1029, Kun 657 passed / 4 skipped), but stop-gate review rejected: settings UI not mounted, IPC/preload handlers not wired, host badge not integrated. Remediation1 full gate green but rejected: real SSH connector hardcodes localhost, no exec/stop/resume IPC/preload/API, no real-host path possible | — | Remediation2 same id / same dirty tree; real-host smoke remains operator-only only after code contract is complete |
