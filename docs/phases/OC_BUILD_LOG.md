...
| M4a | Mobile Pairing Host | `oc-m4a-pairing` | dsv4-pro max | 🟡 | $0.7373 / in=621,867 out=253,420 cacheRead=67,958,272 cache 99.1% | Worker reported READY and full mechanical gate was green in `../ocx-m4a` (root typecheck/lint/tests 157 files/1368 tests, Kun typecheck/tests 58 files passed / 1 skipped and 658 tests passed / 4 skipped, build, `git diff --check`), but orchestrator source review rejected it: settings QR dialog generated fake `Math.random()` codes and placeholder LAN hosts, no real IPC bridge/listener lifecycle was wired, TLS listener audit emission was a no-op, and scope tests duplicated local allow/deny logic rather than exercising the listener. Remediation ordered before merge. | — | Opt-in TLS LAN listener, QR pairing, device scopes; manual phone proof deferred to M-PROOF if automated gates pass |
