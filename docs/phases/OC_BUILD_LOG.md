...
| M4a | Mobile Pairing Host | `oc-m4a-pairing` | dsv4-pro max | 🟡 | $1.4952 total: initial $0.7373 / in=621,867 out=253,420 cacheRead=67,958,272 cache 99.1%; remediation1 $0.7579 / in=967,261 out=157,263 cacheRead=55,249,408 cache 98.3% | Initial run failed source review for fake QR/no IPC/no audit/no real scope tests. First remediation then passed the full independent mechanical gate (root typecheck/lint/tests 157 files/1376 tests, Kun typecheck/tests 58 files passed / 1 skipped and 658 tests passed / 4 skipped, build, `git diff --check`) and fixed the fake QR/listener lifecycle/audit/test gaps, but source review still failed stop gates: LAN listener returns only an acknowledgement instead of serving/proxying the app-server protocol, and unauthenticated `/mobile/qr-payload` remains exposed on the listener despite the token-required surface rule. Second remediation ordered before merge. | — | Opt-in TLS LAN listener, QR pairing, device scopes; manual phone proof deferred to M-PROOF if automated gates pass |
