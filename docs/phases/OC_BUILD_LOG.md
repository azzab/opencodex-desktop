...
| M2.5 | Provider Experience Parity | `oc-m2-5-provider-ux`; retry `oc-m2-5-provider-ux-r2` | dsv4-pro max | 🟡 | $1.4678 total: initial $0.6256 / in=608,827 out=223,575 cacheRead=45,858,304 cache 98.7%; incomplete remediation $0.5158 / in=735,345 out=92,665 cacheRead=31,801,856 cache 97.7%; aborted wrong-cwd recovery $0.3264 / in=600,573 out=43,546 cacheRead=7,525,120 cache 92.6% | Initial worker run reviewed: full mechanical gate green in `../ocx-m2-5` (root typecheck/lint/tests 155 files/1327 tests, Kun typecheck/tests 58 files passed / 1 skipped and 658 tests passed / 4 skipped, build, `git diff --check`, strict secret-literal grep 0 hits), but source review failed the stop gate because per-task assignments are persisted in settings/UI without being consumed by chat routing/Kun runtime. Same-session remediation stopped mid-thought with no READY report; partial worker output was checkpointed on the lane as `46fd35d`; fresh retry `oc-m2-5-provider-ux-r2` dispatched from the clean `../ocx-m2-5` worktree. | — | Evidence paths included: `docs/reference-evidence/providers/`; live multi-provider proof deferred to M-PROOF |
